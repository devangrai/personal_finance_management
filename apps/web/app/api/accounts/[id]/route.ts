...
/**
 * PATCH a Plaid-linked Account. Currently only supports toggling
 * excludeFromNetWorth — the other fields on Account are mostly
 * sourced from Plaid and shouldn't be user-editable.
 */
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: getErrorMessage(error, "Unable to update account.") },
      { status: 500 }
    );
  }
}
