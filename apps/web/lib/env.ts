...
type AppEnv = {
  ...
  cronSecret?: string;
  // SnapTrade — optional because they're only needed when a user wants
  // to use the SnapTrade flow. If unset, /api/snaptrade/* will 400.
  snapTradeClientId?: string;
  snapTradeConsumerKey?: string;
  snapTradeWebhookSecret?: string;
};
...
export function getAppEnv(): AppEnv {
  ...
  return {
    ...
    snapTradeClientId: optionalEnv("SNAPTRADE_CLIENT_ID"),
    snapTradeConsumerKey: optionalEnv("SNAPTRADE_CONSUMER_KEY"),
    snapTradeWebhookSecret: optionalEnv("SNAPTRADE_WEBHOOK_SECRET")
  };
}
