...
export async function getInvestmentsSummary() {
  ...
  // Totals skip excluded accounts; the accounts[] array still shows all of
  // them so the UI can render them with an "excluded" indicator.
  const accountsForTotals = accounts.filter((a) => !a.excludeFromNetWorth);
  const totalBalanceCents = accountsForTotals.reduce(
    (sum, account) => sum + decimalStringToCents(account.currentBalance),
    0
  );
  const retirementBalanceCents = accountsForTotals
    .filter((account) => account.bucket === "retirement")
    .reduce((sum, account) => sum + decimalStringToCents(account.currentBalance), 0);
  const taxableBalanceCents = accountsForTotals
